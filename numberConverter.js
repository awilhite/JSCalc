(function() {
  var convert, i, item, list, number, numbers, resultlist, _i, _j, _len, _len2, _len3;
  ...
  for (_i = 0, _len = list.length; _i < _len; _i++) {
    item = list[_i];
    resultlist.push(convert(item));
  }

  for (i = 0, _len2 = resultlist.length; i < _len2; i++) {
    item = resultlist[i];
  ...
  for (_j = 0, _len3 = resultlist.length; _j < _len3; _j++) {
    item = resultlist[_j];
    number += item;
  }
